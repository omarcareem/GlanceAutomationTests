...
public class ManageUser extends BaseTest {
		...
		/*@Test(dataProvider = "GL_UM_09_2", dataProviderClass = GlanceDataProvider.class, priority = 4)
		 public void GL_UM_09(int middlePagination) {
			...
			common.selectDropDuwn(dropDownValue);
				...
				Assert.assertTrue(common.verifyPageNavigationMiddle(middlePagination,rowCount ,pageCount));
	...
	/*@Test(priority = 5)
	 ...
	 public void GL_UM_11(){
	  ...
	  Assert.assertEquals(manageUser.verifyButtonMin(),"fa fa-chevron-down", "Minimize  success");
	  
	  manageUser.buttonMax();
	  Assert.assertEquals(manageUser.verifyButtonMax(),"fa fa-chevron-up", "Maximize  success");
